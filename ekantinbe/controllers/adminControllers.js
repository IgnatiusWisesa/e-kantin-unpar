...
module.exports = {
    ...
    if (
      ...
      db.query(sqlAddStand, data, (err, addProfileResult) => {
        ...
        if (addProfileResult.insertId === 0) {
          return res.status(200).send({ error: true, message: "Data tidak berhasil di tambah!" });
        } else {
  ...
  adminEditProfileStand: (req, res) => {
    ...
    // Validation Data
    if (standName === undefined || standName === "" || standContact === undefined || standContact === "") {
      ...
      db.query(sqlEditStand, [data, parseInt(profileId)], (err, editStandResult) => {
        ...
        if (editStandResult.affectedRows === 0) {
          return res.status(200).send({ error: true, message: "Data tidak berhasil di update!" });
        } else {
  ...
  adminEditPhotoStand: (req, res) => {
    ...
    upload(req, res, (err) => {
      if (err) return res.status(500).send({ error: err.message, message: "Upload foto gagal!" });
